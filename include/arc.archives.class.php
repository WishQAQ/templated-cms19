...
$this->Fields['body'] = preg_replace('/<img(.+?)height=(.+?) (.+?)>/i',"<img$1$3>",$this->Fields['body']);
    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            else
            {
				...
				$this->PreNext['preurl'] = "javascript:;";
            ...
            else
            {
                ...
                $this->PreNext['nexturl'] = "javascript:;";
    ...
    function GetPagebreakDM($totalPage, $nowPage, $aid)
    {
        ...
        else
        {
            if($nPage==1)
            {
                $PageList.="<a href='view.php?aid=$aid'>上一页</a>";
                if($cfg_rewrite == 'Y')
                {
            ...
            {
                $PageList.="<a href='view.php?aid=$aid&pageno=$nPage'>上一页</a>";
                if($cfg_rewrite == 'Y')
                {
        ...
        for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                if($nowPage!=1)
                {
                    $PageList.="<a href='view.php?aid=$aid'>1</a>";
                    if($cfg_rewrite == 'Y')
                    {
                        $PageList = preg_replace("#.php\?aid=(\d+)#i", '-\\1-1.html', $PageList);
                    }
            ...
            else
            {
                ...
                {
                    $PageList.="<a href='#' class=\"cur\">{$n}</a>";
                }
            }
        }
        if($lPage <= $totalPage)
        {
            $PageList.="<a href='view.php?aid=$aid&pageno=$lPage'>下一页</a>";
            if($cfg_rewrite == 'Y')
            {
                $PageList = str_replace(".php?aid=", "-", $PageList);
                $PageList =  preg_replace("#&pageno=(\d+)#i", '-\\1.html', $PageList);
