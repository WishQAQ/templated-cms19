...
class ListView
{
        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                if($row = $this->dsql->GetArray("al"))
                {
                    ...
                    $num = substr_count($row['body'],'<img');
                    if ($num >=3) {
                        $newstyle = slimg1($row['body'],$row['arcurl']);
                        $row['autostyle'] = "<div class=\"news_list tp_b setp1\"><p class=\"title\"><a target=\"_blank\" href=\"{$row['arcurl']}\">{$row['title']}</a></p>{$newstyle}<span class=\"source\">{$time} 来源：{$row['source']}</span></div>";
                    }
                    else{
                        $row['autostyle'] ="<div class=\"news_list tp_a setp1\"><p class=\"title \"><a target=\"_blank\" href=\"{$row['arcurl']}\">{$row['title']}</a></p><a target=\"_blank\" href=\"{$row['arcurl']}\" class=\"thum1\"> <img src=\"{$row['litpic']}\" /> </a><p class=\"txt\"> <a target=\"_blank\" href=\"{$row['arcurl']}\"> {$row['description']}...[详情] </a> </p><span class=\"source\">{$time} 来源：{$row['source']}</span> </div>";
                    }
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
       ...
       /* $maininfo = "<li><span class=\"pageinfo\">共 <strong>{$totalpage}</strong>页<strong>".$this->TotalResult."</strong>条</span></li>\r\n";*/
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a href='".str_replace("{page}",$prepagenum,$tnamerule)."' class='page-prev'>上一页</a>\r\n";
        ...
        {
            $nextpage.="<a href='".str_replace("{page}",$nextpagenum,$tnamerule)."' class='page-next'>下一页</a>\r\n";
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a href='".$purl."PageNo=$prepagenum' class='page-prev'>上一页</a>\r\n";
        ...
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a href='".$purl."PageNo=$nextpagenum' class='page-next'>下一页</a>\r\n";
